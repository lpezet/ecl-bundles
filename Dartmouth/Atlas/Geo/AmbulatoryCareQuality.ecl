...
EXPORT AmbulatoryCareQuality( STRING pDropZoneIP, STRING pDropZonePath = '/var/lib/HPCCSystems/mydropzone' ) := MODULE
	
	SHARED mDropZoneIP := pDropZoneIP;
	SHARED mDropZonePath := pDropZonePath;
	SHARED BaseDataDirectory := mDropZonePath + '/Dartmouth/Atlas/AmbulatoryCareQuality';
	...
	SHARED mAvailableFiles := DATASET( [
		{ 'latest', 'state', 'http://www.dartmouthatlas.org/downloads/research_files/stata/state_hedis_6575ffs.dta.zip', 'state_hedis_6575ffs.dta' },
		{ 'latest', 'hrr', 'http://www.dartmouthatlas.org/downloads/research_files/stata/hrr_hedis_6575ffs.dta.zip', 'hrr_hedis_6575ffs.dta' },
		{ 'latest', 'hsa', 'http://www.dartmouthatlas.org/downloads/research_files/stata/hsa_hedis_6575ffs.dta.zip', 'hsa_hedis_6575ffs.dta' },
		{ 'latest', 'county', 'http://www.dartmouthatlas.org/downloads/research_files/stata/county_hedis_6575ffs.dta.zip', 'county_hedis_6575ffs.dta' }
	], available_file_layout );
	
	SHARED super_file( STRING pVersion ) := '~common::reference::dartmouth::atlas::ambulatorycarequality::' + pVersion;
